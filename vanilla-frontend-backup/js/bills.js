...
async function saveBill() {
  ...
  const bill = {
    customerName,
    customerPhone,
    items: billItems.filter(item => item.desc || item.rate > 0).map(item => ({
  ...
  try {
    ...
    if (res.success) {
      ...
      if (document.getElementById("bill-no")) {
        document.getElementById("bill-no").value = res.data.invoiceNumber;
      }
      updateBillPreview();
      loadBillHistory();
    }
...
async function loadBillHistory() {
  ...
  if (!tbody) return;
  try {
    const res = await apiFetch("/invoices?limit=20");
    if (res.success) {
      loadedInvoices = res.data.items;
      tbody.innerHTML =
        loadedInvoices
          .map(
            (b) => `
        <tr>
          ...
          <td><button class="btn btn-sm btn-outline" onclick="reprintBill('${b._id}')">Reprint</button></td>
        </tr>`
          )
...
function reprintBill(id) {
  const b = loadedInvoices.find((x) => x._id === id);
  if (!b) return;
  billItems = b.items.map(item => ({
  ...
  document.getElementById("bill-no").value = b.invoiceNumber;
  if (document.getElementById("include-gst")) {
    document.getElementById("include-gst").checked = b.gstRate > 0;
  }
  renderBillItems();
  updateBillPreview();
  printBill();
